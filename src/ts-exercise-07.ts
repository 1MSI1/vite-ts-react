...
const printAnalyzeResult = (
    input: AnalyzeResult | string): void => {
        if ( typeof input ==="string") {console.log(input);} else {
const {inputString,numberOfCharacters, isNumberOfCharacterEven } = input;

console.log(`Wynik funkcji printAnalyzeResult: ${inputString} ${numberOfCharacters} ${isNumberOfCharacterEven}`);
        }
